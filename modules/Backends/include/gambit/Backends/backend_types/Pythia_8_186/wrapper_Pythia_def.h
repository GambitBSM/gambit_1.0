...
namespace CAT_3(BACKENDNAME,_,SAFE_VERSION)
{
    ...
    namespace Pythia8
    {
        ...
        {
            return wrapperbase::BEptr->init(idAin, idBin, eCMin);
        }
        
        inline bool Pythia::init(int idAin, int idBin, double eAin, double eBin)
        {
            return wrapperbase::BEptr->init(idAin, idBin, eAin, eBin);
        }
        ...
        {
            return wrapperbase::BEptr->init(idAin, idBin, pxAin, pyAin, pzAin, pxBin, pyBin, pzBin);
        }
        
        inline bool Pythia::init(std::basic_string<char,std::char_traits<char>,std::allocator<char> > LesHouchesEventFile, bool skipInit)
        {
            return wrapperbase::BEptr->init(LesHouchesEventFile, skipInit);
        }
        
        inline bool Pythia::init(std::basic_string<char,std::char_traits<char>,std::allocator<char> > LesHouchesEventFile)
        {
            return wrapperbase::BEptr->init__BOSS(LesHouchesEventFile);
        }
        ...
        {
            wrapperbase::BEptr->statistics(all, reset);
        }
        
        inline void Pythia::statistics(bool all)
        {
            wrapperbase::BEptr->statistics__BOSS(all);
        }
            ...
            slhaInterface(&(wrapperbase::BEptr->slhaInterface_ref__BOSS()))
        {
            ...
            (process).wrapperbase::BEptr->can_delete_wrapper(false);
            ...
            (particleData).wrapperbase::BEptr->can_delete_me(false);
            (rndm).wrapperbase::BEptr->can_delete_wrapper(false);
            (rndm).wrapperbase::BEptr->can_delete_me(false);
            (couplings).wrapperbase::BEptr->can_delete_wrapper(false);
            (couplings).wrapperbase::BEptr->can_delete_me(false);
            (slhaInterface).wrapperbase::BEptr->can_delete_wrapper(false);
            (slhaInterface).wrapperbase::BEptr->can_delete_me(false);
        }
