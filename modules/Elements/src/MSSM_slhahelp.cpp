...
{ 
   ...
   double addmix = mssm->phys.get_Pole_Mixing(type, gauge_index, mass_index);
   return addmix;
}
                                         ...
                                         const SubSpectrum* mssm)
{
   ...
   for(iter it = mass_es_set.begin(); it != mass_es_set.end(); ++it){
      temp_mass_es = *it;
      temp_addmix = get_mass_addmix_for_gauge(gauge_es, temp_mass_es, mssm);
      //select largest 
...
/// Note: when there is family mixing there's no good definition ~t_1, ~t_2
                                               ...
                                               const SubSpectrum* mssm)
{
   ...
   pair_string_ints type_family_massorder = familystate_label[familystate];
...
{   
   ...
   std::string mass_es = identify_mass_es_closest_to_family(familystate,mssm);
                                        ...
                                        const SubSpectrum* mssm) 
{
   ...
   /// extract info from strings via maps
   int mass_index = (mass_label_to_index_type[mass_es]).first;   
   double admix = mssm->phys.get_Pole_Mixing(type_gauge, gauge_index, mass_index);   
...
{      
   ...
   if(family_type!=type_gauge) 
      { /// throw error in gambit
         std::cout << "we need proper errors " << std::endl;
         return -6.66e66;
      }
