...
namespace Gambit
{
  ...
  namespace Graphs
  {
    ...
    void DependencyResolver::resolveVertexBackend(VertexID vertex)
    {
      ...
      const IniParser::ObservableType * auxEntry = findIniEntry(vertex, boundIniFile->getAuxiliaries());
      ...
      {       
        ...
        if (*it == "none")
        {
          ...
          {       
            ...
            solution = solveRequirement(reqsubset,vertex,auxEntry);
        ...
        else
        {
          ...
          solution = solveRequirement(reqs,vertex,auxEntry,*it);
          // Resolve the backend requirement with that function.
          resolveRequirement(solution,vertex);
        }
    ...
    functor* DependencyResolver::solveRequirement(std::vector<sspair> reqs, VertexID vertex, 
     const IniParser::ObservableType * auxEntry, str group)
    {
          ...
          ++itf) 
      {
        ...
        if ( auxEntry != NULL ) depEntry = findIniEntry((*itf)->quantity(), (*auxEntry).backends);
