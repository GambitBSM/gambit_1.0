...
function(add_gambit_executable executablename LIBRARIES)
  ...
  if(MPI_CXX_FOUND)
    string(REGEX MATCH "([^;]*)libmpi_cxx.so" DUMMY "${MPI_CXX_LIBRARIES}")
    set(MPI_CXX_LIBRARY_PATH "${CMAKE_MATCH_1}")
    set(LIBRARIES ${LIBRARIES} -L${MPI_CXX_LIBRARY_PATH} ${MPI_CXX_LIBRARIES})
    if(MPI_CXX_LINK_FLAGS)
      set_target_properties(${executablename} PROPERTIES LINK_FLAGS ${MPI_CXX_LINK_FLAGS})
    endif()
  endif()
  if(MPI_C_FOUND)
    string(REGEX MATCH "([^;]*)libmpi.so" DUMMY "${MPI_C_LIBRARIES}")
    set(MPI_C_LIBRARY_PATH "${CMAKE_MATCH_1}")
    set(LIBRARIES ${LIBRARIES} -L${MPI_C_LIBRARY_PATH} ${MPI_C_LIBRARIES})
    if(MPI_C_LINK_FLAGS)
