...
namespace Gambit
{
  ...
  {
    return Models::modelClaw().get_parent(model);
  }
  ...
  {
    if (not Models::modelClaw().model_exists(model))
    {
      str errmsg = "Error: model \"";
      errmsg += model + "\" is not in the GAMBIT database.";
      errmsg += "\nRecognised models are:" + Models::modelClaw().list_models();
