  ...
  namespace SpecBit
  {
    ...
    void MSSMspectrum_test (bool &result)
    {
      ...
      if(spinfo.find(k4) == spinfo.end())
      {
         ...
         cout << "Original M1:" << clonedspec->runningpars().get_mass_parameter("M1") << endl;
         clonedspec->runningpars().set_override_mass_parameter(-666,"M1");
         cout << "Override M1:" << clonedspec->runningpars().get_mass_parameter("M1") << endl;

         cout << "Original ~e-(1):" << clonedspec->phys().get_Pole_Mass("~e-",1) << endl;
         clonedspec->phys().set_override_Pole_Mass(-667,"~e-",1);
         cout << "Override ~e-(1):" << clonedspec->phys().get_Pole_Mass("~e-",1) << endl;

         cout << "Original ml2(1,1):" << clonedspec->runningpars().get_mass2_parameter("ml2",1,1) << endl;
         clonedspec->runningpars().set_override_mass2_parameter(-668,"ml2",1,1);
         cout << "Override ml2(1,1):" << clonedspec->runningpars().get_mass2_parameter("ml2",1,1) << endl;

         /// Now add some entry that didn't exist before
         cout << "has 'new_entry'? " << clonedspec->runningpars().has_mass_parameter("new_entry") << endl;
         cout << "..." << endl;
         /// Note: if we try to do it like this, it should fail:
         //clonedspec->runningpars().set_override_mass2_parameter(-1234,"new_entry"); // incorrect: safety still on
         clonedspec->runningpars().set_override_mass_parameter(-1234,"new_entry",false); // correct: safety check turned off
         cout << "has 'new_entry'? " << clonedspec->runningpars().has_mass_parameter("new_entry") << endl;
         cout << "new_entry = " << clonedspec->runningpars().get_mass_parameter("new_entry") << endl;
         cout << endl; 

         /// Tests of ordinary 'setter' functions (these actually replace data in the wrapped object) 
