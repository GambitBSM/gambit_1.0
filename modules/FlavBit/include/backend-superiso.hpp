/* SuperIso specific backend header. This contains bits of code
 * that are specific to a SuperIso backend.
 * \author Nazila Mahmoudi
 * \date 2013-07-10
 */

#ifndef SUPERISO_BACKEND_H
#define SUPERISO_BACKEND_H

#include "backend-common.hpp"

// Fill in below with the name of the shared library, which
// LD_LIBRARY_PATH should point to.
#define SUPERISO_LIB "libsuperiso.so"
...
namespace Gambit {
  ...
  namespace Backend {
  	  ...
  	  /*! \brief Interface for SuperIso routines
  	   * \author Nazila Mahmoudi
  	   * Based on Fortran Herwig interface by A. Raklev and A. Kvellestad
  	   */

		class SuperIso : public BackendBitBase {
		public:

			// Constructor
			SuperIso(){
			        _libName = SUPERISO_LIB;
				loadLib();
			}
