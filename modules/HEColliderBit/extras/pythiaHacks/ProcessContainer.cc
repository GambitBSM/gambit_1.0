...
bool SetupContainers::init(vector<ProcessContainer*>& containerPtrs, 
  ...
  if(couplings->isSUSY){ 
    ...
    // Preselected SUSY code vectors, initially empty
    vector<int> codeVectorA;
    vector<int> codeVectorB;
    ...
    // use 1 as a special code to turn on SUSY code vectors:
    if (codeA == 1 || codeB == 1) {
      codeA = 1;
      codeB = 1;
      // Make codeVectorA be the longer list of the two
      if (settings.ivectLength("SUSY:idVectA") 
            > settings.ivectLength("SUSY:idVectB")) {
        codeVectorA = settings.ivect("SUSY:idVectA");
        codeVectorB = settings.ivect("SUSY:idVectB");
      } 
      else {
        codeVectorA = settings.ivect("SUSY:idVectB");
        codeVectorB = settings.ivect("SUSY:idVectA");
      } 

      // Check for "empty" vectors
      if (codeVectorA.size() == 1 && codeVectorA[0] == 0) {
// TODO   // The following two lines don't work... Should I cout instead??
//        infoPtr->errorMsg("Error in SetupContainers::init: empty SUSY codeVectors", 
//                          "Warning: SUSY codes now ignored");
        codeA = 0;
        codeB = 0;
      } 
      else if (codeVectorB.size() == 1 && codeVectorB[0] == 0) {
        codeB = 0;
      }
    }
    ...
    if (SUSYs || settings.flag("SUSY:gg2gluinogluino")) {
      ...
      // First check code vectors
      if (codeA == 1 
            && find(codeVectorA.begin(), codeVectorA.end(), 1000021)
               != codeVectorA.end()) {
        if (codeB == 0
              || find(codeVectorB.begin(), codeVectorB.end(), 1000021)
                 != codeVectorB.end()) {
          sigmaPtr = new Sigma2gg2gluinogluino();
          containerPtrs.push_back( new ProcessContainer(sigmaPtr) );
        }
      }
      // Then check simple code numbers
      else if (codeA == 0 || codeA == 1000021) {
	...
	for (int iso = 1; iso <= 2; ++iso) {
	  ...
	  // First check code vectors
    if (codeA == 1) {
      if(find(codeVectorA.begin(), codeVectorA.end(), abs(id3))
               == codeVectorA.end()
             && find(codeVectorA.begin(), codeVectorA.end(), abs(id4))
               == codeVectorA.end()) continue;
      if(codeB == 1 
             && find(codeVectorA.begin(), codeVectorA.end(), abs(id3))
               == codeVectorA.end()
             && find(codeVectorB.begin(), codeVectorB.end(), abs(id3))
               == codeVectorB.end()) continue;
      if(codeB == 1 
             && find(codeVectorA.begin(), codeVectorA.end(), abs(id4))
               == codeVectorA.end()
             && find(codeVectorB.begin(), codeVectorB.end(), abs(id4))
               == codeVectorB.end()) continue;
      if(codeB == 1
             && find(codeVectorB.begin(), codeVectorB.end(), abs(id3))
               == codeVectorB.end()
	...
	for (int iso = 1; iso <= 2; ++iso) {
	  ...
	  // First check code vectors
    if (codeA == 1) {
      if(find(codeVectorA.begin(), codeVectorA.end(), abs(id))
               == codeVectorA.end()) continue;
      if(codeB == 1 
             && find(codeVectorB.begin(), codeVectorB.end(), abs(id))
               == codeVectorB.end()) continue;
    }
    // Then check simple code numbers
    else {
	...
	for (int iso = 1; iso <= 2; ++iso) {
	  for (int jso = iso; jso >= 1; --jso) {
        ...
        // First check code vectors
        if (codeA == 1) {
          if(find(codeVectorA.begin(), codeVectorA.end(), abs(id1))
                   == codeVectorA.end()
                 && find(codeVectorA.begin(), codeVectorA.end(), abs(id2))
                   == codeVectorA.end()) continue;
          if(codeB == 1 
                 && find(codeVectorA.begin(), codeVectorA.end(), abs(id1))
                   == codeVectorA.end()
                 && find(codeVectorB.begin(), codeVectorB.end(), abs(id1))
                   == codeVectorB.end()) continue;
          if(codeB == 1 
                 && find(codeVectorA.begin(), codeVectorA.end(), abs(id2))
                   == codeVectorA.end()
                 && find(codeVectorB.begin(), codeVectorB.end(), abs(id2))
                   == codeVectorB.end()) continue;
          if(codeB == 1
                 && find(codeVectorB.begin(), codeVectorB.end(), abs(id1))
                   == codeVectorB.end()
	...
	for (int iNeut1 = 1; iNeut1 <= iNeut2; iNeut1++) {
	  iproc++;
    int id3 = coupSUSY->idNeut(iNeut1);
    int id4 = coupSUSY->idNeut(iNeut2);
    // Skip if specific codes not asked for
	  // First check code vectors
    if (codeA == 1) {
      if(find(codeVectorA.begin(), codeVectorA.end(), abs(id3))
	...
	for (int j = 1; j <= 2; ++j) {
	  iproc++;	
    int id3 = coupSUSY->idChar(i);
    int id4 = coupSUSY->idChar(j);
	  // Skip if specific codes not asked for
	  // First check code vectors
    if (codeA == 1) {
      if(find(codeVectorA.begin(), codeVectorA.end(), abs(id3))
