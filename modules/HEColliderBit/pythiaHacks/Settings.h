...
public:
  ...
  // Constructor
  IVect(string nameIn = " ", vector<int> defaultIn = vector<int>(1, 0), 
...
// parms (double), vects (vector of double), ivects (vector of int),
// and words (string).
...
public:
  ...
  bool isIVect(string keyIn) {
    return (ivects.find(toLower(keyIn)) != ivects.end()); }
  ...
  void addIVect(string keyIn, vector<int> defaultIn, bool hasMinIn, 
    bool hasMaxIn, int minIn, int maxIn) { ivects[toLower(keyIn)] 
    = IVect(keyIn, defaultIn, hasMinIn, hasMaxIn, minIn, maxIn); }  
  ...
  vector<int> ivectDefault(string keyIn);
  ...
  map<string, IVect> getIVectMap(string match);
  ...
  void ivect(string keyIn, vector<int> nowIn); 
  ...
  void forceIVect(string keyIn, vector<int> nowIn);
...
private:
  ...
  vector<int> ivectorAttributeValue(string line, string attribute);
