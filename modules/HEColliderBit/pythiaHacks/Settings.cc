  ...
  for (int i = 0; i < int(files.size()); ++i) {
    ...
    while ( getline(is, line) ) {
      ...
      if (tag != "<flag" && tag != "<flagfix" && tag != "<mode"
         ...
         && tag != "<parm" && tag != "<parmfix"
         && tag != "<vect" && tag != "<vectfix"
         && tag != "<ivect" && tag != "<ivectfix"
         && tag != "<word" && tag != "<wordfix" && tag != "<aidx") continue;
      ...
      // Check for occurence of a ivector and add to ivect map.
      } else if (tag == "<ivect" || tag == "<ivectfix") {
        vector<int> value = ivectorAttributeValue( line, "default="); 
        int minVal = intAttributeValue( line, "min=");
        int maxVal = intAttributeValue( line, "max=");
        addIVect( name, value, hasMin, hasMax, minVal, maxVal);
...
bool Settings::readString(string line, bool warn, ostream& os) {
  ...
  else if (isIVect(name)) inDataBase = -4; 
  ...
  // Update ivect map.
    ...
    istringstream vectData(valueString);
    vector<int> value(ivectorAttributeValue(
      "value=\"" + valueString + "\"", "value="));
    if (!vectData) {
      if (warn) os << "\n PYTHIA Error: variable recognized, but its value"
        << " not meaningful:\n   " << line << endl;
...
bool Settings::writeFile(ostream& os, bool writeAll) {
  ...
  map<string, IVect>::iterator ivectEntry = ivects.begin();
  ...
  while (flagEntry != flags.end() || modeEntry != modes.end() 
    ...
    || ivectEntry != ivects.end() || wordEntry != words.end()) {
    ...
    if ( flagEntry != flags.end() 
      ...
      && ( ivectEntry == ivects.end() || flagEntry->first < ivectEntry->first )
    ...
    // Else check if mode is next, and if so print it.
      ...
      && ( ivectEntry == ivects.end() || modeEntry->first < ivectEntry->first ) 
    ...
    // fixed or scientific depending on value.
      ...
      && ( ivectEntry == ivects.end() || parmEntry->first < ivectEntry->first ) 
    ...
    // fixed or scientific depending on value.
      ...
      && ( ivectEntry == ivects.end() || vectEntry->first < ivectEntry->first ) 
    ...
    // Else check if ivect is next, and if so print it; 
    // fixed or scientific depending on value.
    } else if ( ivectEntry != ivects.end()
      && ( wordEntry == words.end() || ivectEntry->first < wordEntry->first ) 
      ) {
      vector<int> valNow = ivectEntry->second.valNow;
      vector<int> valDefault = ivectEntry->second.valDefault;      
      if ( writeAll || valNow != valDefault ) {
        os  << ivectEntry->second.name << " = ";
...
void Settings::list(bool doListAll,  bool doListString, string match,
  ...
  while (flagEntry != flags.end() || modeEntry != modes.end() 
    ...
    // fixed or scientific depending on value.
      ...
      vector<int> valsNow = ivectEntry->second.valNow;
      vector<int> valsDefault = ivectEntry->second.valDefault; 
      int valNow(0), valDefault(0);
      if ( doListAll || (!doListString && valsNow != valsDefault ) 
        || (doListString && ivectEntry->first.find(match) != string::npos) ) {
	for (unsigned int i = 0; i < valsNow.size() || i < valsDefault.size();
	     ++i) {
	  if ( i == 0 )
	    os << " | " << setw(45) << left 
	       << ivectEntry->second.name << right << " |             ";
	  else
	    os << " | " << setw(45) << " " << right << " |             ";
	  for (int j = 0; j < 4; ++j) { 
	    if (i < valsNow.size()) valNow = valsNow[i];
	    if (i < valsDefault.size()) valDefault = valsDefault[i];
	    if (j == 1) valNow = valDefault;  
	    if (j == 2) valNow = ivectEntry->second.valMin;  
	    if (j == 3) valNow = ivectEntry->second.valMax;  
	    if ( (j == 0 && i >= valsNow.size()) 
		 || (j == 1 && i >= valsDefault.size()) 
		 || (j == 2 && !ivectEntry->second.hasMin)
		 || (j == 3 && !ivectEntry->second.hasMax) )
	      os << "            ";
	    else 
	      os << setw(12) << valNow; 
	    if (j == 0) os << " | ";
...
void Settings::resetAll() {
  ...
  // Loop through the ivects table, resetting all entries.
  for (map<string, IVect>::iterator ivectEntry = ivects.begin(); 
    ivectEntry != ivects.end(); ++ivectEntry) {
    string name = ivectEntry->first;
    resetIVect(name);
  }
...
vector<int> Settings::ivect(string keyIn) {
  if (isIVect(keyIn)) return ivects[toLower(keyIn)].valNow; 
...
vector<int> Settings::ivectDefault(string keyIn) {
  if (isIVect(keyIn)) return ivects[toLower(keyIn)].valDefault;
  infoPtr->errorMsg("Error in Settings::ivectDefault: unknown key", keyIn);
  return vector<int>(1, 0);
}
...
map<string, IVect> Settings::getIVectMap(string match) {
  // Make the match string lower case. Start with an empty map.
  match = toLower(match);
  map<string, IVect> ivectMap;  
  // Loop over the ivect map (using iterator).
  for (map<string,IVect>::iterator ivectEntry = ivects.begin();
       ivectEntry != ivects.end(); ++ivectEntry)
    if (ivectEntry->first.find(match) != string::npos) 
      ivectMap[ivectEntry->first] = ivectEntry->second;
  return ivectMap;
}
...
void Settings::ivect(string keyIn, vector<int> nowIn) { 
  if (isIVect(keyIn)) {
    IVect& ivectNow = ivects[toLower(keyIn)];
    ivectNow.valNow.clear();
    for(vector<int>::iterator now = nowIn.begin();
	now != nowIn.end(); now++) {
      if (ivectNow.hasMin && *now < ivectNow.valMin) 
	ivectNow.valNow.push_back(ivectNow.valMin); 
      else if (ivectNow.hasMax && *now > ivectNow.valMax) 
	ivectNow.valNow.push_back(ivectNow.valMax);
      else ivectNow.valNow.push_back(*now); 
    }
  } 
}  
...
void Settings::forceIVect(string keyIn, vector<int> nowIn) { 
  if (isIVect(keyIn)) ivects[toLower(keyIn)].valNow = nowIn; 
}
...
void Settings::resetIVect(string keyIn) {
  if (isIVect(keyIn)) ivects[toLower(keyIn)].valNow 
    = ivects[toLower(keyIn)].valDefault ; 
}
...
// Extract XML ivector value following XML attribute.

vector<int> Settings::ivectorAttributeValue(string line, string attribute) {
  string valString = attributeValue(line, attribute);
