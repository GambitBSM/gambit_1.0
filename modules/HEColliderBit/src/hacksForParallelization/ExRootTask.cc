 ...
 *  \hacker A. Krislock - OKC, Stockholm University
 *  \hack: Change to account for my hack of TTask
 *
 */
...
ExRootTask *ExRootTask::NewTask(TClass *cl, const char *taskName)
{
  ...
  // Hack: Set the pointer pointers to the BreakPoint and BeginTask in TTask
  task->SetBreakPoint(hackBreakPoint);
  task->SetBeginTask(hackBeginTask);
  // ~Hack
