// @(#)root/base:$Id: TTask.cxx 31827 2009-12-11 07:52:09Z brun $
...
TTask::TTask()
{
   ...
   fBreakout    = 0;
   fTasks       = 0;
   hackBreakPoint = &fgBreakPoint;
   hackBeginTask = &fgBeginTask;
}

//______________________________________________________________________________
TTask::TTask(const char* name, const char *title, 
             TTask **trackBeginTask, TTask **trackBreakPoint)
      : TNamed(name,title)
{
...
{	 
   ...
   if(this!=&tt) {	 
      ...
      fActive=tt.fActive;	 
      hackBeginTask=tt.hackBeginTask;
      hackBreakPoint=tt.hackBreakPoint;
   }	 
   return *this;	 
}	 
...
{	 
   ...
   hackBeginTask = other.hackBeginTask;
   hackBreakPoint = other.hackBreakPoint;
}	 
...
void TTask::Continue()
{
   ...
   *hackBreakPoint = 0;
   ...
   (*hackBeginTask)->ExecuteTasks(fOption.Data());

   if (!*hackBreakPoint) {
      (*hackBeginTask)->CleanTasks();
      *hackBeginTask = 0;
   }
}
...
void TTask::ExecuteTask(Option_t *option)
{
   ...
   if (*hackBeginTask) {
      Error("ExecuteTask","Cannot execute task:%s, already running task: %s",GetName(),(*hackBeginTask)->GetName());
      return;
   }
   if (!IsActive()) return;
