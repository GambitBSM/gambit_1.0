...
NSM_low_scale_constraint<Two_scale>::NSM_low_scale_constraint(
   NSM<Two_scale>* model_, const QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
{
   assert(model && "Error: NSM_low_scale_constraint::apply():"
   ...
   MODEL->set_vH(Re((2*MZDRbar)/Sqrt(Sqr(g1) + Sqr(g2))));
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void NSM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<NSM<Two_scale>*>(model_);
}

void NSM_low_scale_constraint<Two_scale>::set_sm_parameters(const QedQcd& oneset_)
{
   oneset = oneset_;
}

const QedQcd& NSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return oneset;
}
...
double NSM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "NSM_low_scale_constraint<Two_scale>::"
 ...
 */
void NSM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "NSM_low_scale_constraint<Two_scale>::"
   ...
   if (mw_pole_sqr < 0.)
      MODEL->get_problems().flag_tachyon(NSM_info::VWp);
