...
\author[dresden]{Alexander Voigt\corref{cor1}}
\ead{Alexander.Voigt@physik.tu-dresden.de}
...
are related by one or more global signs as in
...
example the case in the $Z_3$-violating NMSSM (\code{SMSSM}), where
...
\end{align}
%
with a non-global $\sign X$.  When \fs solves the tree-level EWSB
...
two-loop tadpoles numerically for the CMSSM and NMSSM parameter points
given in \tabref{tab:unit-test-parameter-points}.
%
\begin{table}[tbh]
  \centering
  \begin{tabularx}{\textwidth}{lX}
    ...
    \bottomrule
  \end{tabularx}
  \caption{CMSSM and NMSSM parameter points used for the unit tests
    against Softsusy.
