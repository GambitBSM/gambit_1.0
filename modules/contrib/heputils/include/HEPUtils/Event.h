...
/// @todo ps is just a copy of the pointers in the lists below... Let them manage their own memory.
//      if (!ps.empty()) for (size_t i = 0; i < ps.size(); ++i) delete ps[i];
//      ps.clear();
      if (!_photons.empty()) for (size_t i = 0; i < _photons.size(); ++i) delete _photons[i];
      _photons.clear();
      if (!_electrons.empty()) for (size_t i = 0; i < _electrons.size(); ++i) delete _electrons[i];
      _electrons.clear();
      if (!_muons.empty()) for (size_t i = 0; i < _muons.size(); ++i) delete _muons[i];
      _muons.clear();
      if (!_taus.empty()) for (size_t i = 0; i < _taus.size(); ++i) delete _taus[i];
      _taus.clear();
      /// @todo Really need to store invisibles when we have a dedicated MET variable?
      if (!_invisibles.empty()) for (size_t i = 0; i < _invisibles.size(); ++i) delete _invisibles[i];
      _invisibles.clear();
