  ...
  namespace DarkBit
  {
        ...
        SimpleHist::SimpleHist(int nBins, double Emin, double Emax, bool logscale): nBins(nBins)
        {
            if(logscale)
            {
                if(Emin<=0)
                {
                    DarkBit_error().raise(LOCAL_INFO, "Lower histogram bin limit must be greater than 0 when using logarithmic binning.");
                }
        ...
        void SimpleHist::addHistAsWeights_sameBin(SimpleHist &in)
        {
            ...
            if(in.nBins != nBins)
            {
                DarkBit_error().raise(LOCAL_INFO, "SimpleHist::addHistAsWeights_sameBin requires identically binned histograms.");
            }
            for(int i=0; i<nBins;i++)
            {
