...
namespace Gambit
{
  ...
  namespace PrecisionBit
  {
    ...
    void make_MSSM_precision_spectrum(const Spectrum* &result)
    {
      ...
      HE->phys().set(Par::Pole_Mass, *Dep::prec_mw, "W-");    //FIXME need to add uncertainty too --> check FH papers
      LE->phys().set(Par::Pole_Mass, *Dep::prec_mw, "W-");    //FIXME need to add uncertainty too --> check FH papers
