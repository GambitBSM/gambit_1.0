       ...
       if(is_primary_printer)
       {
          ...
          for(BaseBufferMap::iterator it = all_buffers.begin(); it != all_buffers.end(); it++)
          {
             ...
             if(it->second->is_synchronised())
             {
               ...
               {
                  std::ostringstream errmsg;
                  errmsg << "Error! Inconsistency detected in output dataset lengths during hdf5printer::finalise(). Datasets from buffer "<<it->second->get_label()<<" have length "<<it->second->get_dataset_length()<<", but previous RA datasets had length "<<dset_length<<".";
                  printer_error().raise(LOCAL_INFO, errmsg.str());
               }
             }
          }
