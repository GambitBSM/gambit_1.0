...
lnL = np.array(group[logl])
lnL_isvalid = np.array(group[logl+"_isvalid"],dtype=np.bool)
...
if do_posterior: pmass = P[mask][cuts]

print "x:", x
print "y:", y
if do_posterior: print "P:", pmass
print "x range: ", np.min(x), np.max(x)
print "y range: ", np.min(y), np.max(y)
if do_posterior: print "P range: ", np.min(pmass), np.max(pmass)
