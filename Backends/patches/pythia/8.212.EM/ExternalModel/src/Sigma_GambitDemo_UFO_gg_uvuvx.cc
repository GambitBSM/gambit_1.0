...
#include "Sigma_GambitDemo_UFO_gg_uvuvx.h"
...
void Sigma_GambitDemo_UFO_gg_uvuvx::initProc() 
{
...
void Sigma_GambitDemo_UFO_gg_uvuvx::sigmaKin() 
{
...
double Sigma_GambitDemo_UFO_gg_uvuvx::sigmaHat() 
{
...
void Sigma_GambitDemo_UFO_gg_uvuvx::setIdColAcol() 
{
  if(id1 == 21 && id2 == 21)
  {
  ...
  // Pick color flow
  int ncolor[1] = {2}; 
  if((id1 == 21 && id2 == 21 && id3 == 9000005 && id4 == -9000005))
  {
    vector<double> probs; 
...
double Sigma_GambitDemo_UFO_gg_uvuvx::weightDecay(Event& process, int iResBeg,
    int iResEnd)
{
...
void Sigma_GambitDemo_UFO_gg_uvuvx::calculate_wavefunctions(const int perm[],
    const int hel[])
{
  ...
  FFV1_1(w[2], w[0], pars->GC_11, pars->mdl_Muv, pars->mdl_Wuv, w[5]); 
  FFV1_2(w[3], w[0], pars->GC_11, pars->mdl_Muv, pars->mdl_Wuv, w[6]); 
...
double Sigma_GambitDemo_UFO_gg_uvuvx::matrix_gg_uvuvx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 3; 
