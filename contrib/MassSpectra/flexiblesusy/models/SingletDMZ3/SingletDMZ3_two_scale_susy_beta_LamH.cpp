...
double SingletDMZ3_susy_parameters::calc_beta_LamH_one_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_LamH = Re(oneOver16PiSqr*(0.27*Power(g1,4) + 2.25*Power(g2,4) +
      ...
      *LamH*Sqr(g1) - 9*LamH*Sqr(g2) + 0.9*Sqr(g1)*Sqr(g2) + 12*Sqr(LamH) + 0.5
      *Sqr(LamSH)));
...
double SingletDMZ3_susy_parameters::calc_beta_LamH_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_LamH = Re(twoLoop*(-3.411*Power(g1,6) + 38.125*Power(g2,6) +
      9.435*Power(g1,4)*LamH - 9.125*Power(g2,4)*LamH - 78*Power(LamH,3) -
      ...
      *(25*LamH + 54*Sqr(g2)) + 80*LamH*(-9*LamH + 10*Sqr(g3))) - 72*
      ...
      Power(g2,4) - 5*LamH*Sqr(g2) + 16*Sqr(LamH))) - 2.5*LamH*Sqr(LamSH)));
