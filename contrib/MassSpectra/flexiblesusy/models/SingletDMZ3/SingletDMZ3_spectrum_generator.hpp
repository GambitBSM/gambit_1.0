 ...
 */
template <class T>
void SingletDMZ3_spectrum_generator<T>::run(const softsusy::QedQcd& oneset,
                                const SingletDMZ3_input_parameters& input)
{
   ...
   SingletDMZ3_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   SingletDMZ3_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
