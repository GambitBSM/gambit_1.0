...
// File generated at Fri 26 Aug 2016 15:06:23

/**
 * @file NSM_two_scale_model_slha.cpp
 * @brief NSM model class wrapper for SLHA conversion
 */
...
#define CLASSNAME NSM_slha<Two_scale>
#define LOCALPHYSICAL(p) physical.p

CLASSNAME::NSM_slha(const NSM_input_parameters& input_)
   : NSM<Two_scale>(input_)
   , physical_slha()
 ...
 */
CLASSNAME::NSM_slha(const NSM<Two_scale>& model_,
...
{
   NSM<Two_scale>::calculate_spectrum();
   convert_to_slha();
}
...
const NSM_physical& CLASSNAME::get_physical_slha() const
{
   return physical_slha;
}

NSM_physical& CLASSNAME::get_physical_slha()
{
   return physical_slha;
}
...
std::ostream& operator<<(std::ostream& ostr, const NSM_slha<Two_scale>& model)
{
   model.print(ostr);
   return ostr;
}
