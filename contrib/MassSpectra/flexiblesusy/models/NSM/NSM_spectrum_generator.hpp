...
class NSM_spectrum_generator
   : public NSM_spectrum_generator_interface<T> {
public:
   NSM_spectrum_generator()
      : NSM_spectrum_generator_interface<T>()
      , susy_scale_constraint()
      , low_scale_constraint()
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~NSM_spectrum_generator() {}

   double get_high_scale() const { return 0.; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const NSM_input_parameters&);
 ...
 */
template <class T>
void NSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const NSM_input_parameters& input)
{
   NSM<T>& model = this->model;
   model.clear();
   ...
   NSM_convergence_tester<T> convergence_tester(
   ...
   NSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NSM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void NSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   NSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, susy_scale);
}
