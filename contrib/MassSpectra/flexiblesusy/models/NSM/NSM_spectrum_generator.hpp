...
// File generated at Mon 1 Jun 2015 12:43:07
...
public:
   ...
   void run(const QedQcd& oneset, const NSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "NSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "NSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   NSM<T> model;
   NSM_susy_scale_constraint<T> susy_scale_constraint;
   NSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void NSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const NSM_input_parameters& input)
{
   ...
   NSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   NSM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void NSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   NSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   NSM_parameter_getter parameter_getter;
   Coupling_monitor<NSM<T>, NSM_parameter_getter>
 ...
 */
template <class T>
void NSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
