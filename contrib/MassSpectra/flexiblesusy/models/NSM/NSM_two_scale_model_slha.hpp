...
/**
 * @class NSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class NSM_slha<Two_scale> : public NSM<Two_scale> {
public:
   explicit NSM_slha(const NSM_input_parameters& input_ = NSM_input_parameters());
   explicit NSM_slha(const NSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~NSM_slha();
