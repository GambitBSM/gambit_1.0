 ...
 */
template <class T>
void SingletDM_spectrum_generator<T>::run(const softsusy::QedQcd& oneset,
                                const SingletDM_input_parameters& input)
{
   ...
   SingletDM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   SingletDM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
