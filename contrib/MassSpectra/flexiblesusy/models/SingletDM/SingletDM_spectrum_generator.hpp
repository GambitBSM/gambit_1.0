...
class SingletDM_spectrum_generator
   : public SingletDM_spectrum_generator_interface<T> {
public:
   SingletDM_spectrum_generator()
      : SingletDM_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~SingletDM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const SingletDM_input_parameters&);
   void write_running_couplings(const std::string& filename = "SingletDM_rgflow.dat") const;

private:
   SingletDM_high_scale_constraint<T> high_scale_constraint;
   SingletDM_susy_scale_constraint<T> susy_scale_constraint;
   SingletDM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void SingletDM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const SingletDM_input_parameters& input)
{
   SingletDM<T>& model = this->model;
   model.clear();
   ...
   SingletDM_convergence_tester<T> convergence_tester(
   ...
   SingletDM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : SingletDM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void SingletDM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   SingletDM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
