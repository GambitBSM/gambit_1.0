...
public:
   SingletDM_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~SingletDM_spectrum_generator_interface() {}

   const SingletDM<T>& get_model() const { return model; }
   SingletDM<T>& get_model() { return model; }
   const Problems<SingletDM_info::NUMBER_OF_PARTICLES>& get_problems() const {
   ...
   virtual void run(const softsusy::QedQcd&, const SingletDM_input_parameters&) = 0;
 ...
 */
template <class T>
void SingletDM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
