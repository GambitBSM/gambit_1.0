...
#include "SCDM_two_scale_low_scale_constraint.hpp"
...
SCDM_low_scale_constraint<Two_scale>::SCDM_low_scale_constraint()
...
SCDM_low_scale_constraint<Two_scale>::SCDM_low_scale_constraint(
   SCDM<Two_scale>* model_, const softsusy::QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
void SCDM_low_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: SCDM_low_scale_constraint::apply():"
...
const Eigen::Matrix<std::complex<double>,3,3>& SCDM_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& SCDM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double SCDM_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double SCDM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void SCDM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<SCDM<Two_scale>*>(model_);
}

void SCDM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& oneset_)
{
   oneset = oneset_;
}

const softsusy::QedQcd& SCDM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return oneset;
}

void SCDM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void SCDM_low_scale_constraint<Two_scale>::initialize()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
...
double SCDM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
double SCDM_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
double SCDM_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void SCDM_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
...
void SCDM_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
 ...
 */
void SCDM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "SCDM_low_scale_constraint<Two_scale>::"
   ...
   if (mw_pole_sqr < 0.)
      MODEL->get_problems().flag_tachyon(SCDM_info::VWp);
