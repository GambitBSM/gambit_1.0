...
#ifndef SCDM_TWO_SCALE_INITIAL_GUESSER_H
#define SCDM_TWO_SCALE_INITIAL_GUESSER_H

#include "SCDM_initial_guesser.hpp"
...
/**
 * @class SCDM_initial_guesser<Two_scale>
 * @brief initial guesser for the SCDM
 */

template<>
class SCDM_initial_guesser<Two_scale> : public Initial_guesser<Two_scale> {
public:
   SCDM_initial_guesser(SCDM<Two_scale>*,
                               const softsusy::QedQcd&,
                               const SCDM_low_scale_constraint<Two_scale>&,
                               const SCDM_susy_scale_constraint<Two_scale>&,
                               const SCDM_high_scale_constraint<Two_scale>&);
   virtual ~SCDM_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   SCDM<Two_scale>* model; ///< pointer to model class
   ...
   SCDM_low_scale_constraint<Two_scale> low_constraint;
   SCDM_susy_scale_constraint<Two_scale> susy_constraint;
   SCDM_high_scale_constraint<Two_scale> high_constraint;
