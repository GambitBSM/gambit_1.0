...
#ifndef SCDM_SPECTRUM_GENERATOR_H
#define SCDM_SPECTRUM_GENERATOR_H

#include "SCDM_spectrum_generator_interface.hpp"
#include "SCDM_two_scale_high_scale_constraint.hpp"
#include "SCDM_two_scale_susy_scale_constraint.hpp"
#include "SCDM_two_scale_low_scale_constraint.hpp"
#include "SCDM_two_scale_convergence_tester.hpp"
#include "SCDM_two_scale_initial_guesser.hpp"
...
template <class T>
class SCDM_spectrum_generator
   : public SCDM_spectrum_generator_interface<T> {
public:
   SCDM_spectrum_generator()
      : SCDM_spectrum_generator_interface<T>()
      , solver()
      ...
      , low_scale(0.)
   {}
   virtual ~SCDM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const SCDM_input_parameters&);
   void write_running_couplings(const std::string& filename = "SCDM_rgflow.dat") const;

private:
   RGFlow<T> solver;
   SCDM_high_scale_constraint<T> high_scale_constraint;
   SCDM_susy_scale_constraint<T> susy_scale_constraint;
   SCDM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void SCDM_spectrum_generator<T>::run(const softsusy::QedQcd& oneset,
                                const SCDM_input_parameters& input)
{
   SCDM<T>& model = this->model;
   model.clear();
   ...
   SCDM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   SCDM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void SCDM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   SCDM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
