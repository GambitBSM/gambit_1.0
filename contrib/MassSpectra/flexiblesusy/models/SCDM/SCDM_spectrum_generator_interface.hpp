...
#ifndef SCDM_SPECTRUM_GENERATOR_INTERFACE_H
#define SCDM_SPECTRUM_GENERATOR_INTERFACE_H

#include "SCDM_two_scale_model.hpp"
#include "SCDM_utilities.hpp"
...
template <class T>
class SCDM_spectrum_generator_interface {
public:
   SCDM_spectrum_generator_interface()
      : model()
      ...
      , force_output(false)
   {}
   virtual ~SCDM_spectrum_generator_interface() {}

   const SCDM<T>& get_model() const { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const SCDM_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "SCDM_spectrum.dat") const;
 ...
 */
template <class T>
void SCDM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void SCDM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   SCDM_mass_eigenstates tmp_model(model);
   try {
   ...
   SCDM_parameter_getter parameter_getter;
   Coupling_monitor<SCDM_mass_eigenstates, SCDM_parameter_getter>
 ...
 */
template <class T>
void SCDM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   SCDM_spectrum_plotter plotter;
