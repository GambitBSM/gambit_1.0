...
SCDM_high_scale_constraint<Two_scale>::SCDM_high_scale_constraint()
...
void SCDM_high_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: SCDM_high_scale_constraint::apply():"
          " model pointer must not be zero");

   if (std::fabs(model->get_g1()) > 3.54491) {
#ifdef ENABLE_VERBOSE
      ERROR("SCDM_high_scale_constraint: Non-perturbative gauge "
...
bool SCDM_high_scale_constraint<Two_scale>::check_non_perturbative()
{
   bool problem = false;
...
double SCDM_high_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double SCDM_high_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

const SCDM_input_parameters& SCDM_high_scale_constraint<Two_scale>::get_input_parameters() const
{
   return model->get_input();
}

SCDM<Two_scale>* SCDM_high_scale_constraint<Two_scale>::get_model() const
{
   return model;
}

void SCDM_high_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<SCDM<Two_scale>*>(model_);
}

void SCDM_high_scale_constraint<Two_scale>::set_scale(double s)
{
   scale = s;
}

void SCDM_high_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void SCDM_high_scale_constraint<Two_scale>::initialize()
{
   assert(model && "SCDM_high_scale_constraint<Two_scale>::"
...
void SCDM_high_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "SCDM_high_scale_constraint<Two_scale>::"
          "update_scale(): model pointer is zero.");

   const double currentScale = model->get_scale();
   const SCDM_soft_parameters beta_functions(model->calc_beta());
   ...
   if (errno == ERANGE) {
#ifdef ENABLE_VERBOSE
      ERROR("SCDM_high_scale_constraint<Two_scale>: Overflow error"
