...
SCDM_spectrum_plotter::SCDM_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void SCDM_spectrum_plotter::extract_spectrum(const SCDM_mass_eigenstates& model)
{
   spectrum.clear();
...
void SCDM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("SCDM_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("SCDM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("SCDM_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void SCDM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("SCDM_spectrum_plotter::write_spectrum: "
...
std::valarray<double> SCDM_spectrum_plotter::to_valarray(double v)
{
