...
/**
 * @class SCDM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class SCDM_slha<Two_scale> : public SCDM<Two_scale> {
public:
   explicit SCDM_slha(const SCDM_input_parameters& input_ = SCDM_input_parameters());
   explicit SCDM_slha(const SCDM<Two_scale>&);
   virtual ~SCDM_slha();
   ...
   const SCDM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   SCDM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
...
private:
   SCDM_physical physical_slha; ///< contains the pole masses and mixings in slha convention
