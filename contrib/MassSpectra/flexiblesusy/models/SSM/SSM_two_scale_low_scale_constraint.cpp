...
SSM_low_scale_constraint<Two_scale>::SSM_low_scale_constraint(
   SSM<Two_scale>* model_, const softsusy::QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
{
   assert(model && "Error: SSM_low_scale_constraint::apply():"
...
void SSM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
...
{
   assert(model && "SSM_low_scale_constraint<Two_scale>::"
...
double SSM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "SSM_low_scale_constraint<Two_scale>::"
 ...
 */
void SSM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "SSM_low_scale_constraint<Two_scale>::"
   ...
   if (mw_pole_sqr < 0.)
      MODEL->get_problems().flag_tachyon(SSM_info::VWp);
