...
double SSM_soft_parameters::calc_beta_mu2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mu2 = Re(twoLoop*(4.1775*Power(g1,4)*mu2 - 9.0625*Power(g2,4)*mu2
      ...
      traceYeAdjYeYeAdjYe - 13.5*mu2*traceYuAdjYuYuAdjYu - 36*mu2*traceYuAdjYu*
      Lambdax + 4.25*mu2*traceYuAdjYu*Sqr(g1) + 7.2*mu2*Lambdax*Sqr(g1) + 11.25
      *mu2*traceYuAdjYu*Sqr(g2) + 36*mu2*Lambdax*Sqr(g2) + 1.125*mu2*Sqr(g1)*
      Sqr(g2) + 40*mu2*traceYuAdjYu*Sqr(g3) + traceYdAdjYd*(0.25*mu2*(-144*
      Lambdax + 5*Sqr(g1) + 45*Sqr(g2) + 160*Sqr(g3)) - 6*Sqr(K1)) - 6*K2*Sqr(
      K1) - 6*traceYuAdjYu*Sqr(K1) - 18*Lambdax*Sqr(K1) + 0.3*Sqr(g1)*Sqr(K1) +
      1.5*Sqr(g2)*Sqr(K1) - 0.25*traceYeAdjYe*(-3*mu2*(-16*Lambdax + 5*Sqr(g1)
      + 5*Sqr(g2)) + 8*Sqr(K1)) - 2*MS*Sqr(K2) - 0.5*mu2*Sqr(K2) - 15*mu2*Sqr(
      Lambdax)));
