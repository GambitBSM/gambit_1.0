...
// File generated at Fri 26 Aug 2016 15:06:41
...
public:
   SSM_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~SSM_spectrum_generator_interface() {}

   const SSM<T>& get_model() const { return model; }
   SSM<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const SSM_input_parameters&) = 0;
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
