...
/**
 * @file SSM_two_scale_model_slha.cpp
 * @brief SSM model class wrapper for SLHA conversion
 */
...
#define CLASSNAME SSM_slha<Two_scale>
#define LOCALPHYSICAL(p) physical.p

CLASSNAME::SSM_slha(const SSM_input_parameters& input_)
   : SSM<Two_scale>(input_)
   , physical_slha()
 ...
 */
CLASSNAME::SSM_slha(const SSM<Two_scale>& model_,
...
{
   SSM<Two_scale>::calculate_spectrum();
   convert_to_slha();
}
...
const SSM_physical& CLASSNAME::get_physical_slha() const
{
   return physical_slha;
}

SSM_physical& CLASSNAME::get_physical_slha()
{
   return physical_slha;
}
...
std::ostream& operator<<(std::ostream& ostr, const SSM_slha<Two_scale>& model)
{
   model.print(ostr);
   return ostr;
}
