...
double SSM_soft_parameters::calc_beta_MS_one_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_MS = Re(4*oneOver16PiSqr*(3*LambdaS*MS + K2*mu2 + Sqr(K1)));
...
double SSM_soft_parameters::calc_beta_MS_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_MS = Re(twoLoop*(-24*K2*mu2*traceYuAdjYu + 4.8*K2*mu2*Sqr(g1) +
      24*K2*mu2*Sqr(g2) - 20*K2*Sqr(K1) - 24*LambdaS*Sqr(K1) - 24*traceYuAdjYu*
      Sqr(K1) + 4.8*Sqr(g1)*Sqr(K1) + 24*Sqr(g2)*Sqr(K1) - 24*traceYdAdjYd*(K2*
      mu2 + Sqr(K1)) - 8*traceYeAdjYe*(K2*mu2 + Sqr(K1)) - 2*MS*Sqr(K2) - 8*mu2
      *Sqr(K2) - 120*MS*Sqr(LambdaS)));
