 ...
 */
 ...
 double DoubleMatrix::diagonaliseSym(ComplexMatrix & v, DoubleVector & w) const {
   ...
   if (displayRows() != c || w.displayEnd() != c || v.displayCols() != c
     ...
     ostringstream ii;
     ii << "Error: Trying to diagonalise matrix \n" << *this;
     ii << "with complex matrix of size " << v.displayRows() << "*" << v.displayCols();
     ii << " into a vector of size " << w.displayEnd() << endl;
     throw ii.str();
   }
 #endif
   // Numerical recipes routine replaces argument, so make a copy of elements
...
double ComplexMatrix::nonHermiticity() const {
#ifdef ARRAY_BOUNDS_CHECKING
   if (displayRows() != displayCols()) {
     ostringstream ii;
     ii << "Trying to calculate hermiticity of matrix in "
	<< "ComplexMatrix::nonHermiticity"
	<< *this
	<< " but it's not square\n";
...
/// Again, only for Hermitian matrices
 double ComplexMatrix::diagonaliseHerm(ComplexMatrix & v, DoubleVector & w) 
   ...
   if (displayRows() != displayCols()) {
     ostringstream ii;
     ii << "Trying to diagonalise hermitian matrix in "
	<< "ComplexMatrix::diagonaliseHerm"
	<< *this
	<< " but it's not square\n";     
     throw(ii.str());
   }
   if (nonHermiticity() > EPSTOL) {
     ostringstream ii;
     ii << "Matrix " << *this << "is too non-Hermitian\n";
     throw(ii.str());
   }
#endif   
   DoubleMatrix r(makeHermitianRealForDiag());
   DoubleVector t(r.displayRows());
  ...
  for (int i=1; i<=w.displayEnd(); i++) {
    ...
    w(i) = t(i * 2);
    int col = 1;
    for (int j=1; j<=w.displayEnd(); j++) {
      v(i, j) = Complex(vr(i, col), vr(i + w.displayEnd(), col));
      col = col + 2;
    }
  }

  ComplexMatrix diagVals(displayRows(), displayCols());
  for (int i=1; i<=displayRows(); i++) diagVals(i, i) = w(i);

  return diagVals.compare(ComplexMatrix(v.hermitianConjugate() * *this * v)) 
    / w.max();
}
