...
\column{0.12\linewidth}
  \centering
  \visible<2->{\includegraphics[width=\linewidth]{Higgs}}\vspace{5mm}
  \visible<4->{\includegraphics[width=\linewidth]{DM}\\
  and friends$++$}
\end{columns}
\vspace{5mm}
...
\begin{frame}
  \frametitle{Background: ways to detect dark matter}
  \begin{itemize}
  \item
  Direct detection -- nuclear collisions and recoils -- CDMS, XENON, DAMA, CRESST, CoGeNT
  \item
  \visible<3->{Direct production -- missing $E_\mathrm{T}$ or otherwise -- LHC, Tevatron}
  \item
  \visible<5->{Indirect detection -- annihilations producing
  \begin{itemize}
    \footnotesize
    \item{\alert<6>{gamma-rays -- \emph{Fermi}, HESS, CTA}}
    \item{anti-protons -- PAMELA, AMS}
    \item{anti-deuterons -- GAPS}
    \item{\alert<6>{neutrinos -- IceCube}, ANTARES}
    \item{$e^+e^-$ -- PAMELA, \emph{Fermi}, ATIC, AMS}\\
    {$\rightarrow$ secondary radiation: Compton$^{-1}$,\\synchrotron, bremsstrahlung}
    \item{\alert<6>{secondary impacts on the CMB}}
  \end{itemize}}
  \item
  \visible<7->{Dark stars -- JWST, VLT}
  \end{itemize}
  
  \begin{textblock}{70}(40,40)
    \only<2>{\includegraphics[width=0.8\linewidth]{lux}}
  \end{textblock}

  \begin{textblock}{65}(32,32)
    \only<4>{\includegraphics[width=\linewidth]{susy_sim3}}
  \end{textblock}
...
\begin{frame}
\frametitle{How to find dark matter with neutrino telescopes}

\begin{columns}[c]
\column{0.75\textwidth}
The short version: \begin{enumerate}
\item \visible<2->{Dark matter in Galactic halo crashes into the Sun}
