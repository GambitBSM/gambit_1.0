...
#include "Sigma_GambitDemo_UFO_uux_uvuvx.h"
...
void Sigma_GambitDemo_UFO_uux_uvuvx::initProc() 
{
...
void Sigma_GambitDemo_UFO_uux_uvuvx::sigmaKin() 
{
...
double Sigma_GambitDemo_UFO_uux_uvuvx::sigmaHat() 
{
...
void Sigma_GambitDemo_UFO_uux_uvuvx::setIdColAcol() 
{
  if(id1 == 2 && id2 == -2)
  {
  ...
  // Pick color flow
  int ncolor[1] = {2}; 
  if((id1 == 2 && id2 == -2 && id3 == 9000005 && id4 == -9000005))
  {
    vector<double> probs; 
  ...
  else if((id1 == -2 && id2 == 2 && id3 == 9000005 && id4 == -9000005))
  {
    vector<double> probs; 
    double sum = jamp2[0][0] + jamp2[0][1]; 
    for(int i = 0; i < ncolor[0]; i++ )
...
double Sigma_GambitDemo_UFO_uux_uvuvx::weightDecay(Event& process, int iResBeg,
    int iResEnd)
{
...
void Sigma_GambitDemo_UFO_uux_uvuvx::calculate_wavefunctions(const int perm[],
    const int hel[])
{
  ...
  FFS1_0(w[3], w[1], w[5], pars->GC_52, amp[1]); 


}
double Sigma_GambitDemo_UFO_uux_uvuvx::matrix_uux_uvuvx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 2; 
