...
f = h5py.File("~/scanResults/gambit_output.hdf5",'r')
group = f["/"]
...
print "Number of valid 'm0' entries    : ", np.sum(m0_isvalid), "(of ", m0_isvalid.shape[0], ")"
print "Index of last valid 'm0' entry (1-indexed) : ", np.max(np.arange(1,m0.shape[0]+1)[m0_isvalid])
print "Total number of 'm12' entries : ", m12.shape[0]
print "Number of valid 'm12' entries : ", np.sum(m12_isvalid), "(of ", m12_isvalid.shape[0], ")"
print "Index of last valid 'm12' entry (1-indexed) : ", np.max(np.arange(1,m12.shape[0]+1)[m12_isvalid])
...
mask = m0_isvalid & m12_isvalid & P_isvalid
...
#cuts = (15<m0[mask]) & (m0[mask]<25) & (m12[mask]<6)
cuts = (0<P[mask])
x = m0[mask][cuts]
y = m12[mask][cuts]
pmass = P[mask][cuts]
...
fig.savefig('collider_multinest_hdf5_posterior.pdf')
